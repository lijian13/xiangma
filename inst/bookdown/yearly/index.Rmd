...
本年度热度最高的三本书是`r paste0("《", hotdf2$doubantitle[1:3], "》", collapse = "
")`，被阅读次数分别为`r paste0(hotdf2$count[1:3], collapse = "、")`次。热度排名前10的书籍如表\@ref(tab:tbl01)所示。
...
其中“本年度阅读次数”是指本年度群友提交的书评中包含该书籍的次数，如果是同一本书的不同版本，会合并成一本。“累计阅读次数”是指截至目前该书籍被阅读并提交书评的次数。
...
此处的读者指的是“响马读书群”提交过书评的活跃群友，每个月读者人数的变化如图\@ref(fig:pic03)所示。

```{r pic03, fig.showtext = TRUE, fig.cap='月度读者数', out.width='70%', fig.width=12, fig.heigth=3, fig.align='center', echo=FALSE, message=FALSE}
par(mar = c(4,4,1,1))
barplot(sapply(split(commdf2$openid, f = substr(commdf2$time,1,7)), FUN = function(X) length(unique(X))), ylab = "读者人数")
```

## 阅读量均匀程度

每位读者的阅读量不同，有的群友读的书非常多，有的群友每个月刚好完成任务，这就造成了阅读量的不均匀。我们使用洛伦兹曲线来衡量，横轴表示按照阅读量排序后，累积的读者数目的百分比，纵轴表示相对应的阅读书籍数的累积百分比。理想的情况下完全均匀，那么该曲线是一条45度的直线，实际情况中会弯曲，弯曲的程度越大，表示不均匀的程度越大。如图\@ref(fig:pic04)所示。

```{r pic04, fig.showtext = TRUE, fig.cap='月度读者数', out.width='70%', fig.width=6, fig.heigth=6, fig.align='center', echo=FALSE, message=FALSE}
par(mar = c(4,4,1,1))
